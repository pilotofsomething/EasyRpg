...
object EasyRpgCommand {
	...
	private fun displayStats(source: ServerCommandSource, player: PlayerEntity?): Int {
		if(player == null) {
			source.sendError(Text.translatable("easyrpg.command.player_null"))
			return 0
		}
		if(source.player != player) {
			source.sendFeedback({ Text.translatable("easyrpg.command.player.stats", player.name) }, false)
		} else source.sendFeedback({ Text.translatable("easyrpg.command.player.stats.self") }, false)

		val rpg = RPG_PLAYER.get(player)

		source.sendFeedback({ Text.translatable("easyrpg.command.player.stats.level", rpg.level) }, false)
		source.sendFeedback({ Text.translatable("easyrpg.command.player.stats.total_xp", rpg.xp) }, false)
		if(rpg.level < config.players.maxLevel) {
			source.sendFeedback(
				{ Text.translatable("easyrpg.command.player.stats.xp_level", rpg.xpForLevel, rpg.xpReqForLevel) },
				false
			)
			source.sendFeedback(
				{ Text.translatable("easyrpg.command.player.stats.xp_tnl", rpg.xpReqForLevel - rpg.xpForLevel) },
				false
			)
		} else {
			source.sendFeedback(
				{ Text.translatable("easyrpg.command.player.stats.xp_level.max", rpg.xpForLevel, rpg.xpReqForLevel) },
				false
			)
			source.sendFeedback(
				{ Text.translatable("easyrpg.command.player.stats.xp_tnl.max", rpg.xpReqForLevel - rpg.xpForLevel) },
				false
			)
		}
		...
		for(stat in IRpgPlayer.Stats.values()) {
			source.sendFeedback(
				{
					Text.literal("  ").append(
						Text.translatable(stat.statName).append(Text.literal(String.format(" %d", rpg.getStat(stat))))
					)
				}, false
			)
		}
	...
	private fun setLevel(source: ServerCommandSource, player: PlayerEntity?, level: Int): Int {
		...
		source.sendFeedback(
			{ Text.translatable("easyrpg.command.player.set_level", player.name.string, level) }, true
		)
		return 1
	}
	...
	private fun setXp(source: ServerCommandSource, player: PlayerEntity?, xp: Long): Int {
		...
		rpg.level = 1
		rpg.xp = xp
		source.sendFeedback({ Text.translatable("easyrpg.command.player.set_xp", player.name.string, xp) }, true)
		return 1
	}
	...
	private fun addLevel(source: ServerCommandSource, player: PlayerEntity?, amount: Int): Int {
		...
		rpg.xp = rpg.xpReqTotal
		source.sendFeedback(
			{ Text.translatable("easyrpg.command.player.add_level", player.name.string, amount) }, true
		)
		return 1
	}
	...
	private fun addXp(source: ServerCommandSource, player: PlayerEntity?, amount: Long): Int {
		...
		rpg.addXP(amount)
		source.sendFeedback(
			{ Text.translatable("easyrpg.command.player.add_xp", player.name.string, amount) }, true
		)
		return 1
	}
